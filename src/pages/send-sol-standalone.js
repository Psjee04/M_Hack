'use client'; // Keep this pragma if it was in the original, Next.js uses it

// import CounterProgram from "@/components/CounterProgram"; // Assuming you will copy/adapt this separately
import WalletButton from '@/components/WalletButton'; // Using existing WalletButton
import { useState, useCallback } from 'react';
import { useConnection, useWallet } from '@solana/wallet-adapter-react';
import { LAMPORTS_PER_SOL, PublicKey, SystemProgram, Transaction } from '@solana/web3.js';
import Head from 'next/head'; // Added for page title

export default function SendSolStandalonePage() { // Renamed export
  ...
  }, [publicKey, receiver, amount, connection, sendTransaction]);

  return (
    <>
      <Head>
        <title>Solana dApp | Send SOL & Counter</title> 
      </Head>
      <div className="flex min-h-screen flex-col items-center justify-center p-8 bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-blue-950">
        ...
        <main className="w-full max-w-md mb-12">
          {/* <CounterProgram /> */} 
          <p className="text-center text-gray-500 dark:text-gray-400">(Counter Program UI would be here)</p>
        </main>
        ...
        <section className="my-8 max-w-2xl w-full">
          ...
          {!connected ? ( 
            <p className="text-center text-orange-500 dark:text-orange-400">Please connect your wallet to send SOL.</p>
          ) : (
